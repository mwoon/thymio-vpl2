...
/*!
  \brief Type5Exercise defines the layout and logic for an exercise of type 5.

  Type 5 exercises are programming exercises where players have to create a VPL program starting with an empty program. A task description is given before the exercise is started but can be reviewed again by clicking the button labelled "Ask Ada what to do".
